...
- **수집 소스 설정(관리자)**: 관리자는 웹 UI에서 크롤 대상 소스(URL·소스 이름·활성 여부·렌더 모드 등)를 등록·수정·비활성화할 수 있다. 설정 변경 시 이후 크롤·이벤트 추출부터 반영된다.

### 5.1.1 크롤링·수집 상세 (현재 스펙)

- **실데이터 추출**: 수집 대상 URL을 실제로 요청한 뒤, HTML에서 제목·본문·발행일을 추출하여 News로 저장한다. 추출은 BeautifulSoup(bs4)를 사용하며, 본문은 article/main/body 우선, 메타(og:title, article:published_time 등)로 제목·날짜를 보완한다.
- **정적 vs JS 렌더링**: 소스별로 **render_mode**(`static` | `browser`)를 둔다. `static`은 서버가 내려준 HTML만으로 처리하고, `browser`는 헤드리스 브라우저(Playwright)로 페이지를 렌더한 뒤 동일한 추출 로직을 적용한다. DB의 CrawlSource에 render_mode 컬럼으로 구분한다.
- **robots.txt 준수**: 각 호스트(origin)별로 robots.txt를 조회하여 허용된 URL만 수집한다. robots.txt 내용은 **캐시**하며, 재시작 후에도 유지하기 위해 **영속화**한다. 백엔드는 파일(JSON) 또는 Redis 중 설정으로 선택 가능하다(`WORKER_ROBOTS_CACHE_BACKEND`, `WORKER_ROBOTS_CACHE_PATH`).
- **크롤링 안전장치**: 요청 전 지연(Crawl-delay 또는 기본값), 동일 호스트 연속 요청 시 지연, 429/503 시 대기 후 1회 재시도, 여러 수집 소스 처리 시 소스 간 지연을 적용하여 과도한 요청을 피한다.
